...
>**중요** 현재 Azure Machine Learning Studio에서 모델을 웹 서비스로 작성하고 배포할 수 있지만 Studio.y 대시보드에서 테스트할 수는 없습니다. 따라서 정리 전 단계의 마지막 섹션을 제외한 모든 단계를 완료할 수 있습니다. 이 변경 내용으로 업데이트됩니다.
...
1. 사용자 인터페이스를 진행하는 데 필요한 경우 다음**을 사용하여 다음 설정을 사용하여 **새 자동화된 ML 작업을 만듭니다.
   ...
   **작업 유형 및 데이터**:
    ...
    - **** 제한: *이 섹션 확장*
        ...
        - **최대 동시 평가판**: 3
        - **최대 노드**: 3
        - **메트릭 점수 임계값**: 0.085(*모델이 0.085 이하의 정규화된 제곱 평균 오차 메트릭 점수를 달성하면 작업이 종료됩니다.*)
        - timeout=120
        - **반복 시간 제한**: 15
        - 조기 종료를 사용하도록 설정합니다.
    - 유효성 검사 및 테스트
...
1. **모델** 탭에서 **배포** 버튼을 누르고 **웹 서비스에 배포** 옵션을 사용하여 다음 설정으로 모델을 배포합니다.
    - **이름**: predict-rentals
    - **설명**: 자전거 대여 예측
    - **컴퓨팅 형식**: Azure Container Instances
    - 인증 사용: 선택됨

1. 배포가 시작될 때까지 대기합니다. 이 작업에는 몇 초 정도 걸릴 수 있습니다. **예측 임대** 엔드포인트에 **대한 배포 상태** 페이지의 기본 부분에 실행* 중으로 *표시됩니다.
1. 상태가 학습 성공으로 변경될 때까지 기다립니다. 이 작업은 최대 5분 정도 걸릴 수 있습니다.

## 배포된 서비스 테스트

>**중요** Azure Machine Learning Studio는 현재 배포 테스트를 사용하는 데 필요한 데이터 세트 만들기 유형을 지원하지 않습니다. 해결 방법이 있을 때 업데이트하겠습니다. 
